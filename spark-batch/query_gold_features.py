...
gold_df.createOrReplaceTempView("gold_user_features")
...
spark.sql("""
    SELECT
        ...
        add_to_cart_count,
        purchase_count,
        ROUND(add_to_cart_count / total_events, 2) AS cart_ratio,
        ROUND(purchase_count / total_events, 2) AS purchase_ratio
    FROM gold_user_features
    WHERE total_events > 0
    ORDER BY cart_ratio DESC, purchase_ratio DESC
    LIMIT 20
""").show(truncate=False)
